...
- Build: 0 warnings, 0 errors; Tests: 85/85 passed
### ElBruno.LocalLLMs v0.6.1 → v0.7.1 Upgrade
- **Date:** 2026-03-28
- **Change:** Bumped ElBruno.LocalLLMs from 0.6.1 to 0.7.1 in MCPToolRouter.csproj
- **API change:** v0.7.1 splits context window info: `ConfigMaxSequenceLength` (raw config value, e.g. 131072) vs `MaxSequenceLength` (effective runtime limit = min(config, options))
- **Sample updates:** LLMDistillationMax and LLMDistillationDemo now display both `ConfigMaxSequenceLength` and `MaxSequenceLength` so users can see config vs effective values
- **No library code changes:** MaxPromptLength stays at 300 chars default per previous decision — we do NOT use model metadata for truncation control
- **Build:** 0 warnings, 0 errors; Tests: 85/85 passed
- **This fixes:** elbruno/ElBruno.LocalLLMs#5 (MaxSequenceLength returned config value instead of effective limit)

### DirectML GPU Runtime Migration (ONNX GenAI Samples)
- **Date:** 2026-03-28
- **Task:** Switched sample projects from CPU-only ONNX GenAI runtime to DirectML (GPU-accelerated runtime for Windows)
- **Changed projects:**
  - `src/samples/LLMDistillationMax/LLMDistillationMax.csproj`
  - `src/samples/LLMDistillationDemo/LLMDistillationDemo.csproj`
- **Package change:** Replaced `Microsoft.ML.OnnxRuntimeGenAI` v0.12.2 → `Microsoft.ML.OnnxRuntimeGenAI.DirectML` v0.12.2
- **Verified:** All other sample projects use ElBruno.LocalLLMs indirectly and don't require direct ONNX package changes
- **Library:** ElBruno.LocalLLMs v0.7.1 already defaults to `ExecutionProvider.Auto` which tries GPU first and falls back to CPU — no code changes needed
- **Build:** Full solution build successful (net8.0 + net10.0 where applicable)
- **Tests:** 115/115 passed on net8.0
- **Impact:** Samples will now leverage GPU acceleration (AMD/Intel/NVIDIA on Windows) for ONNX inference, significantly improving performance for distillation workflows
...
- **Team:** Tron (Core Dev) + Ram (DevRel) + Coordinator (30 new unit tests: 16 PromptDistiller + 14 ToolRouter)
